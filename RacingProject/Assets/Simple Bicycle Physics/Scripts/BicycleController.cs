...
namespace SBPScripts
{
    ...
    public class BicycleController : MonoBehaviour
    {
        ...
        [Header("Inputs")]
        public float horizontal_Input = 0.0f;        // 좌우 키 입력
            ...
            else
            {
                if (wayPointSystem.recordingState == WayPointSystem.RecordingState.Playback)
                {
                    if (isNotStart)
                    {
                        ...
                        if (wayPointSystem.movementInstructionSet.Count - 1 > frame / wayPointSystem.frameIncrement)
                        {
                            transform.position = Vector3.Lerp(transform.position, wayPointSystem.bicyclePositionTransform[frame / wayPointSystem.frameIncrement], Time.deltaTime * wayPointSystem.frameIncrement);
                            transform.rotation = Quaternion.Lerp(transform.rotation, wayPointSystem.bicycleRotationTransform[frame / wayPointSystem.frameIncrement], Time.deltaTime * wayPointSystem.frameIncrement);
                            WayPointInput(wayPointSystem.movementInstructionSet[frame / wayPointSystem.frameIncrement].x, ref customSteerAxis, 5, 5, false);
                            WayPointInput(wayPointSystem.movementInstructionSet[frame / wayPointSystem.frameIncrement].y, ref customAccelerationAxis, 1, 1, false);
                            WayPointInput(wayPointSystem.movementInstructionSet[frame / wayPointSystem.frameIncrement].x, ref customLeanAxis, 1, 1, false);
                            WayPointInput(wayPointSystem.movementInstructionSet[frame / wayPointSystem.frameIncrement].y, ref rawCustomAccelerationAxis, 1, 1, true);
                            sprint = wayPointSystem.sprintInstructionSet[frame / wayPointSystem.frameIncrement];
                            bunnyHopInputState = wayPointSystem.bHopInstructionSet[frame / wayPointSystem.frameIncrement];
                        }
                    }
        ...
        //----------------------------------------------------------------------------------------------------------------------------------------------------
        //Input Manager Controls
        float CustomInput_Horizontal(ref float axis, float sensitivity, float gravity, bool isRaw)
        {
            var r = horizontal_Input;
            var s = sensitivity;
            var g = gravity;
            var t = Time.unscaledDeltaTime;
        ...
        float CustomInput_Vertical(ref float axis, float sensitivity, float gravity, bool isRaw)
        {
            var r = vertical_Input;
            var s = sensitivity;
            var g = gravity;
            var t = Time.unscaledDeltaTime;
